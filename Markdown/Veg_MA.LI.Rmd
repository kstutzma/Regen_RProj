...
ma.nmds.plot <-  ggplot(ma.site.scrs, aes(x=NMDS1, y=NMDS2))+
  geom_point(aes(NMDS1, NMDS2, colour = factor(Treat_Type), shape = factor(Region)), size = 2)+
  scale_color_manual(values =  c("#D8B70A", "#02401B", "#A2A475", "#81A88D", "#972D15"))+
  coord_fixed()+
  theme_classic()+
  theme(panel.background = element_blank()) +
  labs(colour = "Treatment Type", shape = "Region")+
  theme(legend.position = "right", legend.text = element_text(size = 16), legend.title = element_text(size = 18), axis.text = element_text(size = 16))
...
ma.sig_envr.scrs[1,3] <- "Moss cover"
ma.sig_envr.scrs[2,3] <- "Pitch pine basal area"
...
```{r, include=FALSE}
# create ellipse plot -------------------
mali.plot.e <- gg_ordiplot(mali_3d, groups = ma.meta.data_veg$Treat_Type, choices = c(1,3))
...
```{r, echo=FALSE}
ma.13_envr <- envfit(mali_3d, ma.meta.data_veg, choices=c(1,3))
ma13_spc.envr <- envfit(mali_3d, rel_mali5, choices=c(1,3))

# site data -------------------
ma.site.scrs.13 <- as.data.frame(scores(mali_3d, display = "sites"))

ma.site.scrs.13 <- cbind(ma.site.scrs.13, Treat_Type = ma.meta.data_veg$Treat_Type)
ma.site.scrs.13 <- cbind(ma.site.scrs.13, Region = ma.meta.data_veg$Region)

# species data   -------------------
ma.spp.scrs.13 <- as.data.frame(scores(ma13_spc.envr, display = "vectors"))
ma.spp.scrs.13 <- cbind(ma.spp.scrs.13, Species = rownames(ma.spp.scrs.13))
ma.spp.scrs.13 <- cbind(ma.spp.scrs.13, pval = ma13_spc.envr$vectors$pvals)

sig.ma_spp.scrs.13 <- subset(ma.spp.scrs.13, pval<=0.05)

print(sig.ma_spp.scrs.13)

# create species vectors for indicator species -------------------
mali.ind.spp13 <- ma.spp.scrs.13 %>% 
  filter(Species %in% c('QUPR', "KAAN", "RUSP", "SMGL"))


# envr data -------------------
ma.envr.scrs.13 <- as.data.frame(scores(ma.13_envr, display = "vectors"))
ma.envr.scrs.13 <- cbind(ma.envr.scrs.13, env.variables = rownames(ma.envr.scrs.13))
ma.envr.scrs.13 <- cbind(ma.envr.scrs.13, pval = ma.13_envr$vectors$pvals)
ma.sig_envr.scrs.13 <- subset(ma.envr.scrs.13, pval<=0.05)

print(ma.sig_envr.scrs.13)

ma.sig_envr.scrs.13 <- ma.sig_envr.scrs.13 %>% 
  filter(env.variables != "BA_HA")

# base plot -------------------
ma.nmds.plot1.3 <-  ggplot(ma.site.scrs.13, aes(x=NMDS1, y=NMDS3))+
  geom_point(aes(NMDS1, NMDS3, colour = factor(ma.site.scrs.13$Treat_Type), shape = factor(ma.site.scrs.13$Region)), size = 2)+
  scale_color_manual(values =  c("#D8B70A", "#02401B", "#A2A475", "#81A88D", "#972D15"))+
  coord_fixed()+
  theme_classic()+
  theme(panel.background = element_blank()) +
...
ma.nmds.plot1.3 +
  geom_path(data = ellipse.mali, aes(x=x, y=y, color=Group),  linejoin = "round", show.legend = FALSE)+
  geom_segment(data = mali.ind.spp13, aes(x=0, xend=NMDS1, y=0, yend=NMDS3), arrow = arrow(length = unit(0.25, "cm")), colour = ind.sp.mali_color, lwd = 0.75)+
  ggrepel::geom_text_repel(data = mali.ind.spp13, aes(x=NMDS1, y=NMDS3, label = Species), cex = 3, direction = "both", segment.size = 0.25)+
  geom_segment(data = ma.sig_envr.scrs.13, aes(x = 0, xend = NMDS1, y = 0, yend=NMDS3), arrow = arrow(length = unit(0.25, "cm")))+
  ggrepel::geom_text_repel(data = ma.sig_envr.scrs.13, aes(x=NMDS1, y=NMDS3, label = env.variables), cex = 4, direction = "both", segment.size = 0.25) +
  labs(title = "Coastal barrens ordination with indicator \n species & environmental factors (Axes 1 & 3)")+
  theme(plot.title=element_text(hjust=0.5, size=20))
```
...
ma.nmds.plot1.3 +
  geom_path(data = ellipse.mali, aes(x=x, y=y, color=Group),  linejoin = "round", show.legend = FALSE)+
  geom_segment(data = sig.ma_spp.scrs.13, aes(x=0, xend=NMDS1, y=0, yend=NMDS3), arrow = arrow(length = unit(0.25, "cm")), colour = "purple", lwd = 0.25)+
  ggrepel::geom_text_repel(data = sig.ma_spp.scrs.13, aes(x=NMDS1, y=NMDS3, label = Species), cex = 3, direction = "both", segment.size = 0.25)+
  geom_segment(data = ma.sig_envr.scrs.13, aes(x = 0, xend = NMDS1, y = 0, yend=NMDS3), arrow = arrow(length = unit(0.25, "cm")))+
  ggrepel::geom_text_repel(data = ma.sig_envr.scrs.13, aes(x=NMDS1, y=NMDS3, label = env.variables), cex = 4, direction = "both", segment.size = 0.25) +
...
```{r, include=FALSE}
# create ellipse plot -------------------
mali.plot.e <- gg_ordiplot(mali_3d, groups = ma.meta.data_veg$Treat_Type, choices = c(2,3))
...
```{r, echo=FALSE}
ma.23_envr <- envfit(mali_3d, ma.meta.data_veg, choices=c(2,3))
ma23_spc.envr <- envfit(mali_3d, rel_mali5, choices=c(2,3))

# site data -------------------
ma.site.scrs.23 <- as.data.frame(scores(mali_3d, display = "sites"))

ma.site.scrs.23 <- cbind(ma.site.scrs.23, Treat_Type = ma.meta.data_veg$Treat_Type)
ma.site.scrs.23 <- cbind(ma.site.scrs.23, Region = ma.meta.data_veg$Region)

# species data   -------------------
ma.spp.scrs.23 <- as.data.frame(scores(ma23_spc.envr, display = "vectors"))
ma.spp.scrs.23 <- cbind(ma.spp.scrs.23, Species = rownames(ma.spp.scrs.23))
ma.spp.scrs.23 <- cbind(ma.spp.scrs.23, pval = ma23_spc.envr$vectors$pvals)

sig.ma_spp.scrs.23 <- subset(ma.spp.scrs.23, pval<=0.05)

print(sig.ma_spp.scrs.23)

# create species vectors for indicator species -------------------
mali.ind.spp.23 <- ma.spp.scrs.23 %>% 
  filter(Species %in% c('QUPR', "KAAN", "RUSP", "SMGL"))



# envr data -------------------
ma.envr.scrs.23 <- as.data.frame(scores(ma.23_envr, display = "vectors"))
ma.envr.scrs.23 <- cbind(ma.envr.scrs.23, env.variables = rownames(ma.envr.scrs.23))
ma.envr.scrs.23 <- cbind(ma.envr.scrs.23, pval = ma.23_envr$vectors$pvals)
ma.sig_envr.scrs.23 <- subset(ma.envr.scrs.23, pval<=0.05)

print(ma.sig_envr.scrs.23)


# base plot -------------------
ma.nmds.plot2.3 <-  ggplot(ma.site.scrs.23, aes(x=NMDS2, y=NMDS3))+
  geom_point(aes(NMDS2, NMDS3, colour = factor(ma.site.scrs.23$Treat_Type), shape = factor(ma.site.scrs.23$Region)), size = 2)+
  scale_color_manual(values =  c("#D8B70A", "#02401B", "#A2A475", "#81A88D", "#972D15"))+
  coord_fixed()+
  theme_classic()+
  theme(panel.background = element_blank()) +
...
ma.nmds.plot2.3 +
  geom_path(data = ellipse.mali, aes(x=x, y=y, color=Group), linejoin = "round", show.legend = FALSE)+
  geom_segment(data = mali.ind.spp.23, aes(x=0, xend=NMDS2, y=0, yend=NMDS3), arrow = arrow(length = unit(0.25, "cm")), colour = ind.sp.mali_color, lwd = 0.75)+
  ggrepel::geom_text_repel(data = mali.ind.spp.23, aes(x=NMDS2, y=NMDS3, label = Species), cex = 3, direction = "both", segment.size = 0.25)+
  geom_segment(data = ma.sig_envr.scrs.23, aes(x = 0, xend = NMDS2, y = 0, yend=NMDS3), arrow = arrow(length = unit(0.25, "cm")))+
  ggrepel::geom_text_repel(data = ma.sig_envr.scrs.23, aes(x=NMDS2, y=NMDS3, label = env.variables), cex = 4, direction = "both", segment.size = 0.25) +
  labs(title = "Coastal barrens ordination with indicator \n species & environmental factors (Axes 2 & 3)")+
  theme(plot.title=element_text(hjust=0.5, size=20))
```
...
ma.nmds.plot2.3 +
  geom_path(data = ellipse.mali, aes(x=x, y=y, color=Group), linejoin = "round", show.legend = FALSE)+
  geom_segment(data = sig.ma_spp.scrs.23, aes(x=0, xend=NMDS2, y=0, yend=NMDS3), arrow = arrow(length = unit(0.25, "cm")), colour = "purple", lwd = 0.25)+
  ggrepel::geom_text_repel(data = sig.ma_spp.scrs.23, aes(x=NMDS2, y=NMDS3, label = Species), cex = 3, direction = "both", segment.size = 0.25)+
  geom_segment(data = ma.sig_envr.scrs.23, aes(x = 0, xend = NMDS2, y = 0, yend=NMDS3), arrow = arrow(length = unit(0.25, "cm")))+
  ggrepel::geom_text_repel(data = ma.sig_envr.scrs.23, aes(x=NMDS2, y=NMDS3, label = env.variables), cex = 4, direction = "both", segment.size = 0.25) +
  labs(title = "Coastal barrens ordination with significant \n species & environmental factors (Axes 2 & 3)")+
  theme(plot.title=element_text(hjust=0.5, size=20))
