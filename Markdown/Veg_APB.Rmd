...
```{r, warning=FALSE}
apb.veg_envr.data <- envfit(apb_2d, apb.meta_for.veg) #getting big SD is 0 warning
...
apb.nmds.plot + labs(title = "APB basic ordination plot")
...
apb.nmds.plot +
  ...
  labs(title = "APB Ordination with environmental vectors")
```
Could lower the pval to reduce # of species, range from 0.004 - 0.044


### Ellipse - there is a problem with chol(cov) within veganCovEllipse function - need to find another way
```{r, eval=FALSE}




df_ell.apb <- data.frame()

apb.site.scrs$Treat_Type <- as.factor(apb.site.scrs$Treat_Type)


for(i in levels(apb.site.scrs$Treat_Type)){
  df_ell.apb <- rbind(df_ell.apb, cbind(as.data.frame(with(apb.site.scrs [apb.site.scrs$Treat_Type==i,],
...
NMDS.mean.veg=aggregate(apb.site.scrs[ ,c("NMDS1", "NMDS2")], 
                         list(group = apb.site.scrs$Treat_Type), "mean")
 
# data for labelling the ellipse
NMDS.mean=aggregate(apb.site.scrs[,c("NMDS1", "NMDS2")], 
                    list(group = apb.site.scrs$Treat_Type), "mean")

apb.nmds.plot+
  geom_path(data = df_ell.apb, aes(x=NMDS1, y=NMDS2, color = Treat_Type), show.legend = FALSE)
```
...
apb.nmds.plot+
  #geom_path(data = df_ell.apb, aes(x=NMDS1, y=NMDS2, color = Treat_Type), show.legend = FALSE) +
  geom_segment(data = sig.apb_spp.scrs, aes(x=0, xend=NMDS1, y=0, yend=NMDS2), arrow = arrow(length = unit(0.25, "cm")), colour = "purple", lwd = 0.3)+
  ggrepel::geom_text_repel(data = sig.apb_spp.scrs, aes(x=NMDS1, y=NMDS2, label = Species), cex = 3, direction = "both", segment.size = 0.25)+
  geom_segment(data = apb.sig_envr.scrs, aes(x = 0, xend = NMDS1, y = 0, yend=NMDS2), arrow = arrow(length = unit(0.25, "cm")))+
  ggrepel::geom_text_repel(data = apb.sig_envr.scrs, aes(x=NMDS1, y=NMDS2, label = env.variables), cex = 4, direction = "both", segment.size = 0.25) +
  labs(title = "APB Ordination plot")
```
