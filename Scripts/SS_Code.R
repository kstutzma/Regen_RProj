...
rm(SS_Control_Data.csv1, SS_Fall_Burn_Data.csv2, SS_Mow_Burn_Data.csv3, SS_Spring_Burn_Data.csv4)
...
# Lets look at small seedling and add the missing rows and 0s ; actually, I need to set the species groups next -------------------
...
# yes bitch. this is right -------------------


# sort by treatment type -------------------

SpRx_SS <- filter(ss_merge, Treat_Type == "SpringRx")

FaRx_SS <- filter(ss_merge, Treat_Type == "FallRx")

MowRx_SS <- filter(ss_merge, Treat_Type == "MowRx")

Cntr_SS <- filter(ss_merge, Treat_Type == "Control")
...
# should i be expanding these to per hectare observations? -------------------
